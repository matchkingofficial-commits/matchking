...
function startPredictionCountdown() {
  ...
  predictionTimer = setInterval(() => {
    ...
    if (countdownSeconds <= 0) {
      ...
      if (container) {
        container.innerHTML =
          '<div class="no-predictions">⏳ Recalculating… next signal in 20s</div>';
      }
...
function checkTerminalAccess() {
  ...
  if (!token || !userEmail) {
    console.warn(
      "⛔ Unauthorized session context detected. Redirecting to login portal...",
    );
    clearLocalSession();
...
function startWebSocket() {
  ...
  if (!isAuthenticated) {
    console.warn(
      "⚠️ Cannot start stream processing without a valid local login session.",
    );
    return false;
  }
  ...
  try {
    ...
    console.log(
      `🔗 Initializing real-time public data feed pipeline: ${wsEndpoint}`,
    );
  } catch (error) {
    console.error(
      "❌ Failed to construct WebSocket stream target instance:",
      error,
    );
    return false;
  }
  ...
  derivWs.onopen = function () {
    console.log(
      `✅ Pipeline successfully locked onto feed data for ${activeMarkets.length} markets.`,
    );
...
function updateMarketsList() {
  ...
  if (marketsElement) {
    const activeCount = Object.keys(marketsData).filter(
      (symbol) => marketsData[symbol].tickHistory.length > 0,
    ).length;
...
function updateAggregatedPrice() {
  ...
  if (latestPrice !== null && marketsData[latestSymbol]) {
    ...
    if (priceElement)
      priceElement.textContent = latestPrice.toFixed(
        marketsData[latestSymbol].decimalPlaces,
      );
    if (symbolElement) symbolElement.textContent = latestSymbol;

    const lastDigitElement = document.getElementById("last-digit");
    if (lastDigitElement) {
      const priceString = latestPrice.toFixed(
        marketsData[latestSymbol].decimalPlaces,
      );
...
function updatePredictionHighlights() {
  ...
  const marketsWithFullData = Object.keys(marketsData).filter(
    (symbol) => marketsData[symbol].tickHistory.length >= requiredTicks,
  );
  const totalActiveMarkets = Object.keys(marketsData).length;

  if (marketsWithFullData.length === 0) {
    if (container)
      container.innerHTML = `<div class="no-predictions">Fetching historical data (${requiredTicks} ticks per market)...</div>`;
    return;
  ...
  console.log(
    `📊 Analysis ready: All ${totalActiveMarkets} markets have ${requiredTicks} ticks for accurate predictions`,
  );
  ...
  for (let digit = 0; digit <= 9; digit++) {
    ...
    const overallPercentage =
      totalTicks > 0 ? (totalCount / totalTicks) * 100 : 0;

    if (overallPercentage >= threshold || digitMarkets.length > 0) {
      console.log(
        `🎯 Digit ${digit}: Overall ${overallPercentage.toFixed(2)}%, Markets above threshold: ${digitMarkets.length}`,
      );
      predictions.push({
        digit: digit,
  ...
  predictions.sort(
    (a, b) => parseFloat(b.overallPercentage) - parseFloat(a.overallPercentage),
  );
  ...
  if (topPredictions.length === 0) {
    container.innerHTML =
      '<div class="no-predictions">No repeating patterns detected yet...</div>';
    currentPrediction = null;
  } else {
    ...
    console.log(
      `🎯 New prediction: Digit ${pred.digit}  – 45s display then 30s cooldown`,
    );
  }

  console.log(
    `🎯 Prediction scan: ${predictions.length} digits above ${threshold}%, showing top ${topPredictions.length}`,
  );
