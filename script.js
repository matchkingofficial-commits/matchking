...
const WHATSAPP_NUMBER = "wa.me/254735525483";
...
async function verifyTransaction() {
  ...
  try {
    ...
    if (toAddress.toLowerCase() !== USDT_ADDRESS.toLowerCase()) {
      showVerificationError(
        msgDiv,
        "❌ Transaction sent to wrong address. Please verify and resend to the correct address.",
      );
      verifyBtn.disabled = false;
      verifyBtn.textContent = "Verify Payment";
      return;
    ...
    if (txValue !== currentPurchaseAmount) {
      showVerificationError(
        msgDiv,
        `❌ Incorrect amount. Expected ${currentPurchaseAmount} USDT, but received ${txValue} USDT.`,
      );
      verifyBtn.disabled = false;
      verifyBtn.textContent = "Verify Payment";
      return;
    ...
    showVerificationError(
      msgDiv,
      "Error verifying transaction. Please try again or contact support.",
    );
    verifyBtn.disabled = false;
    verifyBtn.textContent = "Verify Payment";
  }
